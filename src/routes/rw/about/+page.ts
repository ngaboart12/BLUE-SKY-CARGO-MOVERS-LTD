...
export async function load({}) {
	const membersTeams = await client.fetch(
		`*[_type == "membersTeam" && (country == "bi" || country == "all")]`
	);
	...
	const organizationTypes = await client.fetch(
		`*[_type == "organizationType" && (country == "bi" || country == "all")]`
	);
	const openRoles = await client.fetch(
		`*[_type == "openRoles" && isAvailable == true && (country == "bi" || country == "all")]`
	);
	const youtubeUpdates = await client.fetch(
		`*[_type == "youtubeUpdates" && (country == "bi" || country == "all")]`
	);
