...
export async function load({}) {
	const membersTeams = await client.fetch(
		`*[_type == "membersTeam" && (country == "en" || country == "all")]`
	);
	...
	const organizationTypes = await client.fetch(
		`*[_type == "organizationType" && (country == "en" || country == "all")]`
	);
	const openRoles = await client.fetch(
		`*[_type == "openRoles" && isAvailable == true && (country == "en" || country == "all")]`
	);
	const youtubeUpdates = await client.fetch(
		`*[_type == "youtubeUpdates" && (country == "en" || country == "all")]`
	);
