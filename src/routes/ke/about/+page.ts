...
export async function load({}) {
	const membersTeams = await client.fetch(
		`*[_type == "membersTeam" && (country == "ke" || country == "all")]`
	);
	...
	const organizationTypes = await client.fetch(
		`*[_type == "organizationType" && (country == "ke" || country == "all")]`
	);
	const openRoles = await client.fetch(
		`*[_type == "openRoles" && isAvailable == true && (country == "ke" || country == "all")]`
	);
	const youtubeUpdates = await client.fetch(
		`*[_type == "youtubeUpdates" && (country == "ke" || country == "all")]`
	);
