...
export async function load({}) {
	const membersTeams = await client.fetch(
		`*[_type == "membersTeam" && (country == "sa" || country == "all")]`
	);
	...
	const organizationTypes = await client.fetch(
		`*[_type == "organizationType" && (country == "sa" || country == "all")]`
	);
	const openRoles = await client.fetch(
		`*[_type == "openRoles" && isAvailable == true && (country == "sa" || country == "all")]`
	);
	const youtubeUpdates = await client.fetch(
		`*[_type == "youtubeUpdates" && (country == "sa" || country == "all")]`
	);
