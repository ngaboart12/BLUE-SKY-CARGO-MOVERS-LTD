<script lang="ts">
	...
	$: blog = data.blogs?.slice(0, 5).map((value: any) => {
		if (value.mainImage) {
	...
	$: paragraphsPartOne = data.service?.paragraphs?.slice(0, 2) || [];
	$: paragraphsPartTwo = data.service?.paragraphs?.slice(2, 4) || [];
	...
	<div class="absolute top-0 w-full h-full z-10 object-cover overflow-hidden">
		<Carousel dots={false}>
			{#each data.service?.images || [] as item}
				<img src={item} alt="service" class="w-full object-cover h-[70vh]" />
			{/each}
		</Carousel>
	</div>
...
<section class="flex flex-col gap-6 sm:gap-10 pt-20" id="anchor-section">
