...
<script lang="ts">
	...
	const url = 'https://www.smartcleansolutions.com/insights/cleaning-solutions-innovation';
	const title = "Revolutionizing Cleanliness: Innovations in Smart Cleaning Solutions";

	const blog = {
		title: title,
		mainImage: "https://th.bing.com/th/id/R.895a5ef2ee01774c468ff71f9c9c547c?rik=ZI8c21KJoypx1Q&pid=ImgRaw&r=0",
		publishedAt: new Date().toISOString(),
		body: [
			{ _type: 'block', style: 'normal', children: [{ text: 'Smart Cleans Solutions is at the forefront of revolutionizing cleaning and maintenance with cutting-edge technology and innovative solutions.' }] },
			{ _type: 'block', style: 'normal', children: [{ text: 'Our smart cleaning systems are designed to improve efficiency, reduce costs, and enhance the overall cleanliness of your environment.' }] },
			{ _type: 'image', asset: { _ref: ["images/009.jpg"] }, alt: 'Advanced Cleaning Technology' },
			{ _type: 'block', style: 'normal', children: [{ text: 'Smart Cleans Solutions is at the forefront of revolutionizing cleaning and maintenance with cutting-edge technology and innovative solutions.' }] },
	...
	const markdownConverter = blog.body.map((item: any) => {
		if (item._type === 'image') {
			return `<img src="${blog.mainImage}" alt="${item.alt}" class="w-full h-full object-cover my-4" />`;
		} else {
			const style = item.style || 'normal';
			if (/^h\d/.test(style)) {
				const level = style.replace(/[^\d]/g, '');
	...
	export const blogs = [
		{
			_id: '1',
			title: 'How Smart Cleaning Enhances Workplace Productivity',
			description: 'Explore the benefits of smart cleaning systems in boosting workplace efficiency and employee satisfaction.',
			mainImage: ["/images/009.jpeg"],
			publishedAt: new Date().toISOString()
		},
		{
			_id: '2',
			title: 'Eco-Friendly Cleaning Solutions for Modern Businesses',
			description: 'Discover how our eco-friendly cleaning solutions contribute to a healthier environment and sustainable practices.',
			mainImage: ["/images/0001.jpg"],
			publishedAt: new Date().toISOString()
		},
		{
			_id: '3',
			title: 'The Future of Automated Cleaning Technology',
			description: 'A deep dive into the latest advancements in automated cleaning technology and their impact on the industry.',
			mainImage: ["/images/006.jpeg"],
			publishedAt: new Date().toISOString()
		},
		// {
		// 	_id: '4',
		// 	title: 'Maintaining Hygiene Standards with Smart Solutions',
		// 	description: 'Learn how smart cleaning solutions help maintain high hygiene standards in various facilities.',
		// 	mainImage: ["/images/007.jpeg"],
		// 	publishedAt: new Date().toISOString()
		// }
	].slice(0, 4);
</script>
...
<div class="flex flex-col gap-6 sm:gap-10 mt-20">
	<div class="max-w-7xl mx-auto py-8 md:px-8 px-4 w-full flex flex-col items-center">
		<div class="w-full flex gap-10 flex-col sm:flex-row">
			<div class="flex flex-col flex-1 sm:w-full">
				<div class="max-w-xl text-start">
					<Title isSmall name="Revolutionizing Cleanliness: Innovations in Smart Cleaning Solutions"></Title>
				</div>
				<div class="w-full flex flex-col">
					<div class="h-[400px] w-full rounded-xl overflow-hidden mt-8">
						<img src={blog.mainImage} alt={blog.title} class="w-full h-full object-cover" />
